...
int venc_init(int chnId, int width, int height, RK_CODEC_ID_E enType) {
    ...
    if (enType == RK_VIDEO_ID_AVC) {
        ...
        stAttr.stRcAttr.stH264Cbr.u32BitRate = 4 * 1024;   // lighter for RV1103
        ...
        stAttr.stRcAttr.stMjpegCbr.u32BitRate = 4 * 1024;
    }

    stAttr.stVencAttr.enType = enType;
    stAttr.stVencAttr.enPixelFormat = RK_FMT_YUV420SP;  // NV12 for H.264 on RV1103
    if (enType == RK_VIDEO_ID_AVC)
    ...
    stAttr.stVencAttr.u32BufSize = width * height * 3 / 2; // NV12 size
    stAttr.stVencAttr.enMirror = MIRROR_NONE;

    RK_S32 ret = RK_MPI_VENC_CreateChn(chnId, &stAttr);
    if (ret != RK_SUCCESS) {
        printf("ERR: RK_MPI_VENC_CreateChn ret=0x%x\n", ret);
        return ret;
    }
    ...
    if (ret != RK_SUCCESS) {
        printf("ERR: RK_MPI_VENC_StartRecvFrame ret=0x%x\n", ret);
        return ret;
    }
    return 0;
}